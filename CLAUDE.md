...
- **GitHub MCP** ✅ — our own `github` **stdio** server in `.mcp.json` runs the official `ghcr.io/github/github-mcp-server` Docker image via a `bash -c` wrapper that injects the token at launch: `-e GITHUB_PERSONAL_ACCESS_TOKEN="$(/snap/bin/gh auth token)"`. This pulls straight from the `gh` keyring at server start, so **no secret is stored anywhere** and there's no dependency on how `claude` was launched (the earlier `${GITHUB_PAT}` env-var approach failed because `~/.bashrc` only reaches interactive shells, not Claude Code's process). Approved via `enabledMcpjsonServers` in `.claude/settings.json`. The `github` *plugin* is disabled (it sent no credentials → "Failed to connect"). The `gh` CLI works alongside it. Requires Docker (present, runs without sudo). If github shows failing: `docker pull ghcr.io/github/github-mcp-server`, confirm `gh auth token` prints a token, and restart Claude Code (`.mcp.json` + approvals load only at startup).
