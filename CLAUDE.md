...
- 🚕 **Taxi spike roadmap** (PRs A–G on `feat/scene-drivable-taxi-spike`): **A** drivable box ✅ · **B** chase camera ✅ · **C** procedural Peugeot 504-style taxi model ✅ (`TaxiModel.tsx` — went procedural instead of Quaternius to stay $0 and on-brand) · **E** enter/exit + on-foot walk ✅ (`useDrive` store, `Character`, `DriveController`; press **E** near taxi to exit/enter; chase cam + landmark proximity follow `activeRef`) · **next: street geometry** · then taxi-call flow. Real landmarks/character rig still pending (Phases 2–4).
- ⚠️ **Known gaps:** (1) no on-screen "Press E to drive" hint yet — needs an i18n key + HUD (don't ship bare English, see Don't rules); (2) no smoke tests on any `src/features/scene/` 3D component (jsdom can't run WebGL — needs `@react-three/test-renderer`). Both deferred during the spike; revisit before merging to `main`.
