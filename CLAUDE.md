...
- ✅ **Phase 2 so far** (all merged to `main` via CI):
  ...
  - **Casbah structure** (`Casbah.tsx`) — procedural whitewashed-cube cluster (seeded), watchtower, doorway, footprint collider; placed at `[-22,0,-12]` (west, off the cross street). **Visual only — no proximity/panel yet.**
- 🎯 **Next**: (1) **Casbah interactivity** — proximity trigger + a **Projects panel**, mirroring the GrandePoste/About wiring (extend `LandmarkId` with `"casbah"`, add the panel + i18n + tests). (2) The remaining 3 anchors (Notre-Dame d'Afrique, Maqam Echahid, Café Zack) + character rig. (A licensed car/landmark `.glb` in `public/models/` + `useGLTF` can replace any procedural model later.)
